...
User.objects.filter(username__isnull=True) #Takes either True or False, which correspond to SQL queries of IS NULL and IS NOT NULL
...
# if some_queryset.contains(obj) - Returns True if the QuerySet contains obj, and False if not
# if some_queryset.exists() - Returns True if the QuerySet contains any results, and False if not